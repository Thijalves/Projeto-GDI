...
-- TIGGER ANTES DE INSERIR UM POKEMON
CREATE OR REPLACE TRIGGER INSERIR_TREINADOR 
AFTER INSERT ON TREINADOR 
FOR EACH ROW
DECLARE
    NOME_SEQUENCIA VARCHAR(30):= (
        SELECT NOME 
        FROM PERSONAGEM P
        WHERE P.CPF = :NEW.CPF
    ) 
...
CREATE OR REPLACE TRIGGER INSERIR_POKEMON BEFORE
INSERT DESCRITOR ON POKEMON FOR EACH ROW
BEGIN
